...
var repoSetTetsudouConfigCmd = &cobra.Command{
	Use:   "set-tetsudou-config [repo_id] [tetsudou_url] [tetsudou_token]",
	Short: "Set a Tetsudou config for a repo",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		req, err := http.NewRequest(http.MethodPut, server+"/repos/"+args[0]+"/tetsudou", bytes.NewReader(data))
		if err != nil {
			return err
		}
...
func init() {
	repoCmd.AddCommand(repoSetTetsudouConfigCmd)
